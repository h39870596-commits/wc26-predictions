...
- **单日赞助 ¥4.9**:查看今日全部场次的 AI 预测报告,仅当日有效(localStorage 存日期,跨日自动失效)。
- **全程赞助 ¥49**:赛事期间每天自动解锁当天全部预测报告,按日推送直至决赛。
...
3. **生成解锁码**:`python3 scripts/gen_codes.py 20 10`(20 个单日码+10 个全程码,首批已生成)——明文在 `private/codes-private.txt`(已 gitignore,**勿公开**),前端只存 SHA-256 哈希;
...
### 自动开通(支付后免人工发码)—— 后端已就绪,待商户资质

`server/app.py` 是完整的自动开通后端(下单→动态收款二维码→查单确认→签发 token→前端自动解锁并从后端拉取当日内容),**本地模拟模式已端到端联调通过**;内置下单/查询限频、订单过期回收、token 签名校验等防护。上线需要:微信支付商户号(Native 支付)与/或支付宝开放平台当面付资质 + 一个可跑 Python 的部署位(腾讯云函数即可)。部署三步见 `server/README.md`;部署后前端只改 `SITE_CONFIG.payApi` 一处即切换为全自动模式,解锁码/静态收款码自动退场。
